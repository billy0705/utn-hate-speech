...
def change_path(data_dir=DATA_DIR, plots_dir=PLOTS_DIR):
    ...
    print(f"Plots directory set to: {PLOTS_DIR}")

def latex_check():
    print_section("Latex Check")

    # change the global PLOT_STORAGE_FORMAT
    global PLOT_STORAGE_FORMAT
    if shutil.which("pdflatex") is not None:
        try:
            # Test if matplotlib can actually use LaTeX
            ...
            PLOT_STORAGE_FORMAT = "pdf"
        except Exception:
            # If LaTeX support fails, fall back
    ...
    print("Plot format:", PLOT_STORAGE_FORMAT)

def data_loading():
    print("Loading data...")
    print(f"Data directory: {DATA_DIR}")
    df_annotations = pd.read_csv(os.path.join(DATA_DIR, "annotations.csv"))
    df_llm_responses = pd.read_csv(os.path.join(DATA_DIR, "llm_responses.csv"))
    df_hate_samples = pd.read_csv(os.path.join(DATA_DIR, "hate_samples.csv"))

    print(f"Loaded {len(df_annotations)} annotations, {len(df_llm_responses)} responses, {len(df_hate_samples)} samples")
    return df_annotations, df_llm_responses, df_hate_samples
...
def save_plot(fig, filename, dpi=300):
    ...
    plt.tight_layout()
    save_path = ''
    if PLOT_STORAGE_FORMAT == "png":
        save_path = os.path.join(PLOTS_DIR, "image", f"{filename}.png")
    if PLOT_STORAGE_FORMAT == "pdf":
        save_path = os.path.join(PLOTS_DIR, "pdf", f"{filename}.pdf")
...
def merge_data(df_annotations, df_llm_responses, df_hate_samples):
    print_section("Data Preparation")

    for col in label_columns:
        df_annotations[col] = df_annotations[col].apply(canon_label)

    df_annotations['Final_Label'] = df_annotations['Final_Label'].apply(canon_label)
    ...
    df_merged['Is_Safe'] = df_merged['Final_Label'].apply(lambda x: x in safe_labels if pd.notna(x) else False)

    print(f"Data merged successfully. Total records: {len(df_merged)}")
    print(f"Available columns: {list(df_merged.columns)}")
    return df_merged

def dataset_statistics(df_merged):
    print_section("1. Overall Dataset Statistics")

    print("Dataset Composition:")
    print(f"  • Total responses: {len(df_merged):,}")
    print(f"  • Unique samples: {df_merged['Sample_ID'].nunique():,}")
    print(f"  • Languages: {df_merged['Language'].nunique()} ({', '.join(sorted(df_merged['Language'].unique()))})")
    print(f"  • Models: {df_merged['Model_Name'].nunique()} ({', '.join(sorted(df_merged['Model_Name'].unique()))})")
    print(f"  • Hate types: {df_merged['Hate_Type'].nunique()} ({', '.join(sorted(df_merged['Hate_Type'].unique()))})")

    print(f"\nOverall Label Distribution:")
    final_dist = df_merged['Final_Label'].value_counts().sort_index()
    total = len(df_merged)
...
def model_human_agreement(df_annotations):
    print_section("2. Model-Human Agreement Analysis")

    if 'Human_Label' in df_annotations.columns and not df_annotations['Human_Label'].isna().all():
        human_data = df_annotations.dropna(subset=['Human_Label']).copy()
        model_cols = ['Chatgpt_Label', 'Claude_Label', 'Deepseek_Label',
                      'Llama_Label', 'Qwen_Label', 'Final_Label']
        ...
        if agreement_results:
            ...
            sns.heatmap(df, annot=True, fmt="d", cmap="Blues", ax=ax, annot_kws={"size":18}, cbar=False)
            # sns.heatmap(df, annot=True, fmt="d", cmap="Blues", ax=ax, annot_kws={"size":18})
            ax.set_xlabel("Model Prediction", fontsize=16)
            ax.set_ylabel("Human Label", fontsize=16)
            ax.tick_params(axis='x', labelsize=14)
            ax.tick_params(axis='y', labelsize=14)
            # plt.show()
            save_plot(fig, "human_llm_error_heatmap_without_cbar")
...
def safety_analysis(df_merged):
    ...
    safety_by_model = df_merged.groupby('Model_Name').agg({
    ...
    safety_by_language = df_merged.groupby('Language').agg({
        'Is_Safe': ['mean', 'count', 'sum']
    }).round(3)
    ...
    safety_by_target = df_merged.groupby('Hate_Type').agg({
        'Is_Safe': ['mean', 'count', 'sum']
    }).round(3)
...
def counter_speech_analysis(df_merged):
    print_section("4. Counter-Speech Analysis")

    cs_data = []
    for (lang, model), group in df_merged.groupby(['Language', 'Model_Name']):
...
def label_distribution_analysis(df_merged):
    print_section("5. Label Distribution Analysis")

    hate_speech_counts = df_merged[df_merged['Final_Label'] == 'Hate Speech'].groupby('Model_Name').size()
    ...
    if not has_hate_speech:
        ...
        for model, count in hate_speech_counts.items():
            total = len(df_merged[df_merged['Model_Name'] == model])
            rate = count / total
            print(f"  • {model}: {count} cases ({rate:.3f} rate)")

    lang_dist = df_merged.groupby(['Language', 'Final_Label']).size().unstack(fill_value=0)
    ...
    target_dist = df_merged.groupby(['Hate_Type', 'Final_Label']).size().unstack(fill_value=0)
    ...
    final_counts = df_merged['Final_Label'].value_counts(normalize=True).round(3)
...
def inter_model_agreement(df_annotations):
        ...
        for i, col1 in enumerate(model_label_cols):
            for j, col2 in enumerate(model_label_cols):
                if i == j:
                    ...
                    both_labeled = df_annotations.dropna(subset=[col1, col2])
...
def per_model_label_by_language_tables(df_merged):
    ...
    for model, g in df_merged.groupby('Model_Name'):
        ...
        tbl_counts = tbl_counts.reindex(label_order, fill_value=0)
        ...
        ax.set_ylabel('Label')
        save_plot(fig, f"label_by_language_{model.lower()}_heatmap_probs")

        fig, ax = plt.subplots(figsize=(10, 6))
        sns.heatmap(tbl_counts, annot=True, fmt='d', cmap='Blues', ax=ax,
                    cbar_kws={'label': 'Count'}, cbar=False, annot_kws={"size":20})
        ax.set_title(f'Label Counts by Language — {model}', fontsize=16)
        ax.set_xlabel('Language', fontsize=16)
        ax.set_ylabel('Label', fontsize=16)
        ax.tick_params(axis='x', labelsize=14)
        ax.tick_params(axis='y', labelsize=14)
        save_plot(fig, f"label_by_language_{model.lower()}_heatmap_counts")
